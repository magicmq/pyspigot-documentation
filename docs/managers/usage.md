...
- The [Packet Events Manager](core/packetevents.md), for working with PacketEvents.
...
| Packet Events Manager^*^ | `pyspigot.packet_events_manager()` | `from dev.magicmq.pyspigot.manager.packetevents import PacketEventsManager`        |
...
ps.packet_events_manager().<function>
...
2.  Available on the Bukkit (ProtocolLib) and BungeeCord (Protocolize) platforms only.
